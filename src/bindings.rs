...
impl HavanaWrapper {
    ...
    fn get_top_level_accumulators(&self) -> PyResult<Vec<(f64, f64, f64, f64, f64, usize)>> {
        match &self.grid {
            ...
            Grid::DiscreteGrid(ds) => Ok(ds.discrete_dimensions[0]
                .bin_accumulator
                .iter()
                .map(|acc| (acc.avg, acc.err, acc.chi_sq, acc.max_eval_negative, acc.max_eval_positive, acc.num_zero_evals))
                .collect::<Vec<_>>()),
        }
    }

    fn get_current_estimate(&self) -> PyResult<(f64, f64, f64, f64, f64, usize)> {
        match &self.grid {
            Grid::ContinuousGrid(cs) => {
                let mut a = cs.accumulator.shallow_copy();
                a.update_iter();
                Ok((a.avg, a.err, a.chi_sq, a.max_eval_negative, a.max_eval_positive, a.num_zero_evals))
            }
